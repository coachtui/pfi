...
**Per-dimension confidence**: `high` · `moderate` · `limited` · `insufficient_data`, derived deterministically from: history length (<60d → limited cap, <90d → moderate cap), share of the dimension's metrics unavailable (any optional metric missing, e.g. utilization without credit limits → drop one level), categorization quality (uncategorized share of transactions → drop one level for Cash Flow/Stability/Growth/Liquidity, whose metrics derive from transaction categories, including category-derived essential spend for `liquid_runway_months`), unresolved transfers (>5% of in-window transfers on included accounts unpaired → drop one level for Cash Flow/Stability/Growth, whose windowed totals depend on correctly-paired transfers), manually entered data share (>80% of included accounts on the `manual` provider → drop one level for every dimension, applied before the demo cap), and **source reliability** (Plaid Slice 1, DECISIONS #43 — PFI's own facts about its sources, never Plaid's category confidence, applied first so they lead the reasons list): transaction history still loading on any included connected account → cap every dimension at `limited`; any included connected account stale (no successful sync within 35 days of the as-of date) or not `ok` → drop one level for every dimension; a cached synced balance observed before the as-of day → drop one level for the balance-driven dimensions (Liquidity, Debt, Concentration); a non-zero reconciliation discrepancy on the latest sync anchor → drop one level for Cash Flow/Stability/Growth/Liquidity ("some synced transactions may be missing"); over 25% of in-window non-transfer outflows categorized `other` → drop one level for the category-driven dimensions. Demo data caps every dimension at `moderate` with the reason "demo dataset".
