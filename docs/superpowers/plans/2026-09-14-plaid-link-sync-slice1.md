...
- [ ] `delete_connected_item_data(p_item_id uuid) returns jsonb` — same shape: assert ownership, then delete anchors, transactions, batches, accounts of the Item; mark Item `disconnected`. Grant to `authenticated`.
- [ ] `pnpm supabase db push` (or the project's migration apply convention) against the linked project; regenerate nothing (no generated types in this repo).
...
- [ ] DATA_MODEL.md: new tables/columns, the RPCs, and the flag documented as an implementation detail.
...
- [ ] `types.ts`: plain shapes — `PlaidAccountShape` (id, name, official_name, mask, type, subtype, balances {current, available, limit, last_updated_datetime}), `PlaidTransactionShape` (transaction_id, account_id, amount, date, authorized_date, name, merchant_name, pending, personal_finance_category {primary, detailed, confidence_level}, taxonomy version), `SyncPages`, `SyncPlan` (the exact jsonb the RPC consumes: `accounts`, `inserts`, `updates`, `deletes`, `anchors`, `item`, `audit`), `ItemStatus`, `UpdateStatus`.
- [ ] `crypto.ts`: `encryptToken(plaintext, key, version)` / `decryptToken(payload, keys: Record<version, key>)` with Node `webcrypto` AES-256-GCM, random 12-byte IV, base64 `iv||ciphertext||tag`. Tests: round trip; wrong key fails; tampered byte fails; version routing picks the right key; key length enforced.
...
- [ ] `mapAccountType(type, subtype): AccountType` per §8; `mapAccount(shape, institutionName): NewAccountRow` (`provider='plaid'`, `display_name = official_name ?? name`, `mask`, `credit_limit = balances.limit`, `external_account_id`). Table-driven tests for every listed subtype plus unknowns → fallbacks (`brokerage`, `personal_loan`, `other_asset`).
