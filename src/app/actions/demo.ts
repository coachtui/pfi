...
async function clearDemoRows(
  ...
  // Transactions cascade from accounts. Only demo-authored events go; derived
  // events (source = 'derived', DECISIONS #44) belong to real accounts and are
  // regenerated by the rebuild that follows. Snapshots are derived and rebuilt.
  ...
  const del2 = await supabase.from("financial_events").delete().eq("user_id", userId).eq("source", "demo");
