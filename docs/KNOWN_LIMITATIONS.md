...
- **The `FCF === owner-created equity` reconciliation identity holds exactly only for demo data** (zero market drift, static mortgage/property balances). It will need a market-appreciation term once real (non-demo) investment/property holdings arrive.
...
- **`computePeriodStatement`'s reconciliation identity holds by construction for real `buildDailySnapshots` output** (transfers replay as balance deltas), but there is no end-to-end integration test yet feeding real snapshot-builder output through it — carried from Task 3's review as a follow-up.
