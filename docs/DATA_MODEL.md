...
`id`, `user_id (FK)`, `date`, `type` (check: paycheck, bonus, mortgage_payment, large_purchase, insurance_payment, investment_contribution, debt_payment, debt_payoff, tax_payment, unexpected_expense), `label`, `amount (check >= 0, added in 0002)`, `direction`, `created_at`, plus (0016) `source` (`demo` | `derived`, default `demo`), `transaction_id` (FK → transactions, cascade delete; the transaction a derived event came from), `derivation_version` (the `EVENT_DERIVATION_VERSION` that produced a derived row). Two provenances: the demo loader authors `source = 'demo'` rows with the dataset; `rebuildSnapshots` regenerates `source = 'derived'` rows wholesale from the household's effective transactions and recurring series (rules in `docs/DRIVER_EVENTS.md`, DECISIONS #44), never for demo accounts. Partial unique index `(user_id, transaction_id, type) where source = 'derived' and transaction_id is not null` (one event per transaction per type); index `(user_id, source)`. Trigger `financial_events_check_transaction_ownership` (insert and update) rejects a `transaction_id` owned by another user (same pattern as 0008). Migration 0017 adds the FK index `financial_events_transaction_idx`, the provenance check `financial_events_derived_provenance` (derived ⇒ transaction_id + derivation_version present; demo ⇒ both null), and narrows the source index to `(user_id) where source = 'derived'`. `clearDemoData` deletes only `source = 'demo'` rows. Events explain the line; they never feed snapshots or the score.
