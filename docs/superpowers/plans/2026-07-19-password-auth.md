...
- [ ] **Step 2: Supabase password policy** — Supabase dashboard → Authentication → Providers → Email: set **Minimum password length = 8**. **Enable "Confirm email."** (Correction, discovered during Task 9 live testing: this was assumed already-on because magic links "used it," but magic-link/OTP sign-in doesn't actually require it — the project currently has it OFF. With it off, `signUp()` on a duplicate email returns a real "already registered" error instead of the obfuscated response the design relies on; Task 7's `signUpWithPassword` was hardened in a follow-up fix to treat that error identically to success regardless of this setting, but turning "Confirm email" on is still required for the product's own stated flow — "we sent a verification link... verified user lands in onboarding" — to actually hold, since right now sign-up grants an immediate session with no verification step.)
