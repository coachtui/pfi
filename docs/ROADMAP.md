...
## Phase 7 — Account aggregation (opened early 2026-09-14 — Slice 1 merged 2026-09-15 (PR #34, DECISIONS #43); Slice 2 implemented on `worktree-plaid-slice2` (DECISIONS #44), pending review/merge and the owner's Plaid-dashboard steps)
...
- Slice 2 (2026-09-15): production cut-over — `PLAID_REDIRECT_URI` + `/plaid/oauth` return page and a shared Link hook, privacy/terms language + version bump + first-tap disclosure, per-user Item cap (`PLAID_MAX_ITEMS`), Sandbox chip, production runbook/owner checklist; driver-event derivation for real data (`financial_events.source`, pure versioned `deriveEvents`, rebuilt with snapshots — `docs/DRIVER_EVENTS.md`). Real institutions: after the owner's Plaid Production approval.
