...
**Reasoning:** live-verified with a seeded 1050-transaction account split across two import batches (500 + 550 rows): the old unbounded query undercounted the older batch as 450 rows (true count 500) because the newer batch's rows shared the same 1000-row cap — a real user would have seen a wrong row count (and potentially wrong first/last dates) on the `/accounts` "Recent imports" list for any batch whose rows fell past the cap. The paginated version correctly reports 500/550, 1050 total.
**Consequences:** all four known unbounded-`.select()` sites on `transactions` are now paginated. Further audit found other unbounded selects on `daily_snapshots`/`financial_events`/`transactions` in `getDashboardData` and `getReportData` (same file) — not fixed here, added to KNOWN_LIMITATIONS as the next candidates if this cleanup continues. There is still no repo-wide guard (lint rule or wrapper convention) preventing a future unbounded `.select()` from reintroducing this class of bug.
