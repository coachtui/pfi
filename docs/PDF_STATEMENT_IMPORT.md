...
Phase 1 supports fictional/sanitized examples and real statements that expose usable embedded text for:
...
- Corrupted PDFs
- Scanned/image-only statements when OCR is not available
...
The database stores import metadata, a short raw-text excerpt for debugging, parser metadata, validation results, staged rows, and correction/audit rows. Application code must not log full statement text, full account numbers, or unmasked financial details.
