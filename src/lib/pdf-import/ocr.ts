...
function configuredCommand(name: string, fallback: string): string {
...
export class LocalTesseractOcrProvider implements StatementOcrProvider {
  ...
  private async extractInternal(input: { pdfBytes: Uint8Array; importId: string; ownerId: string; pageCount: number | null }): Promise<OcrDocument> {
    const renderCommand = configuredCommand("PDF_IMPORT_RENDER_COMMAND", "pdftoppm");
    await assertCommand(renderCommand, "PDF renderer");
    ...
    try {
      await writeFile(pdfPath, Buffer.from(input.pdfBytes));
      try {
        await execFileAsync(renderCommand, [
          "-png",
          "-r", String(dpi),
      ...
      try {
        ...
        for (const image of images) {
          const pageNumber = Number(image.match(/\d+/)?.[0] ?? pages.length + 1);
          const result = await worker.recognize(path.join(dir, image), {}, { text: true, blocks: true });
